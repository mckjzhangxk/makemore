...
def update_replay_buffer(
    ...
    for timestep in range(len(episode_experience)):
        ...
        if her_type == HERType.FINAL:
            ...
            # get final goal
            final_goal=episode_experience[-1][0]
            # compute new reward
            ...
            random_future=np.random.choice(range(timestep,len(episode_experience)),1)[0]
            
            final_goal=episode_experience[random_future][0]
            # compute new reward
            reward=env_reward_function(state,final_goal)
            ...
            # get random goal
            goal_indexes=np.random.choice(range(len(episode_experience)),num_relabeled)
            for ii in goal_indexes:
                final_goal=episode_experience[ii][0]
                # compute new reward
                reward=env_reward_function(state,final_goal)
                # add to replay buffer
                replay_buffer.add(np.append(state, final_goal),
...
def train(
    ...
    for epoch_idx in range(num_epochs):
        ...
        for _ in range(num_episodes):
            ...
            episode_experience, ep_reward, succeeded = run_episode.run_episode(env, model, steps_per_episode)
