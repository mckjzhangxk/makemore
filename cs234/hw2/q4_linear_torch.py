...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        self.q_network=nn.Linear(img_width*img_height*n_channels*self.config.state_history,num_actions).to(self.device)
        self.target_network=nn.Linear(img_width*img_height*n_channels*self.config.state_history,num_actions).to(self.device)
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        f= self.q_network if network=="q_network" else self.target_network
        out=f(torch.flatten(state,1).to(self.device))
    ...
    def calc_loss(
        ...
        B=q_values.shape[0]
        valid_mask=torch.bitwise_not(done_mask)
        loss=torch.nn.functional.mse_loss(q_values[range(B),actions.long()],rewards-target_q_values.max(1)[0],reduction="none")*valid_mask
