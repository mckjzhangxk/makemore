...
def get_icl_prompts(
    ...
    **Note**: Be sure to shuffle the support examples and labels 
    ...
    support_inputs=[support_inputs[k] for k in perm]
    support_labels=[support_labels[k] for k in perm]
    # YOUR CODE HERE
    prompt = ''
    if prompt_mode=='qa':
        for k in range(len(support_inputs)):
            prompt+=support_inputs[k]+" In the "+support_labels[k]+"."
        prompt+=test_input+" In the"
    elif prompt_mode=="none":
        for q,a in zip(support_inputs,support_labels):
            prompt+=q+" "+a+" "
        prompt+=test_input
    elif prompt_mode=="tldr":
        for k in range(len(support_inputs)):
            prompt+=support_inputs[k]+" TL;DR: "+support_labels[k]
        prompt+=test_input+" TL;DR:"
    else:
        for k in range(len(support_inputs)):
            prompt+="request:"+support_inputs[k]+" response:"+support_labels[k]+"."
        prompt+="request:"+test_input+" response:"
    return prompt
...
def do_sample(model, input_ids, stop_tokens, max_tokens):
    ...
    with torch.inference_mode():
        for _ in range(max_tokens):
            y=model(input_ids).logits[:,-1]
            label=y.argmax(-1,keepdim=True)
            input_ids=torch.concat((input_ids,label),dim=1)
            next_char=(label[0][0].item()) 
            
            if next_char in stop_tokens:break
            sampled_tokens.append(next_char)
                ...
                for k in ks:
                    ...
                    for repeat in range(args.repeats):
                        ...
                        for row in pbar:
                            ...
                            idx=torch.unsqueeze(torch.tensor(input_ids),0).to(DEVICE)
                            result=do_sample(model,idx,stop_tokens,max_tokens)
                            
                            # print(tokenizer.decode(result))
                            
                            decoded_prediction = tokenizer.decode(result)
